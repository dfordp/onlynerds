...
export function ModuleEditor({ courseId, onModuleChange, isCreator }: ModuleEditorProps) {
  ...
  const fetchModules = async () => {
    try {
      ...
      const assessmentPromises = data.map((module: Module) => 
      ...
      const assessmentMap = data.reduce((acc: any, module: Module, index: number) => ({
        ...acc,
                        ...
                        <div className="flex items-center gap-4">
                          ...
                          <div className="flex-grow">
                            {isCreator ? (
                              <Input
                                value={module.name}
                                onChange={(e: React.ChangeEvent<HTMLInputElement>) => handleUpdateModule(module._id, { name: e.target.value })}
